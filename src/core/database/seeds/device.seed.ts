...
export default class DeviceSeeder implements Seeder {
  public async run(
    dataSource: DataSource, // serve para criar instâncias de entidades
  ): Promise<any> {
    const deviceRepository = dataSource.getRepository(Device); // pegamos o repositório da entidade que queremos popular
    ...
    ]); // criamos uma instância da entidade e passamos os dados que queremos popular
    await deviceRepository.save(device); // salvamos a instância no banco de dados
  }
}
